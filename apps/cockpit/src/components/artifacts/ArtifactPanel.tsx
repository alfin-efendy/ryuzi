...
export function ArtifactPanel({ runnerId, sessionPk, refreshKey }: Props) {
  ...
  const load = () => void commands.listSessionArtifacts(runnerId, sessionPk).then((result) => {
    if (result.status === "ok") { setItems(result.data); setError(null); }
    else setError(result.error.message);
  });

  useEffect(load, [runnerId, sessionPk, refreshKey]);

  const download = (artifact: ArtifactInfo) => void commands.fetchArtifact(runnerId, sessionPk, artifact.id).then((result) => {
    if (result.status === "ok") save(result.data);
    else setError(result.error.message);
  });

  return <SettingsCard className="mx-4 mb-3 shrink-0 overflow-hidden">
    <div className="flex items-center justify-between px-3 py-2">
      <div className="flex items-center gap-2 text-sm font-semibold"><FileText aria-hidden size={14} />Artifacts</div>
    ...
    <div className="divide-y divide-border">
      {items.map((artifact) => {
        ...
        return <div key={`${artifact.id}:${artifact.referenceId ?? "source"}`} className="flex min-h-12 items-center gap-2 px-3 py-2 text-xs">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-1 truncate text-muted-foreground">
              {artifact.referenceId ? <><Link2 aria-hidden size={11} />Shared from {artifact.sharedFromSessionPk}</> : <>{artifact.creator} · {artifact.contentType ?? "file"} · {bytes(artifact.sizeBytes)}</>}
            </div>
          </div>
          <Badge variant={unavailable ? "secondary" : "outline"}>{label(artifact.status)}</Badge>
          <Button variant="ghost" size="icon-xs" title="Preview" disabled={unavailable} onClick={() => setPreview(artifact)}><Eye aria-hidden size={14} /></Button>
          <Button variant="ghost" size="icon-xs" title="Download" disabled={unavailable} onClick={() => download(artifact)}><Download aria-hidden size={14} /></Button>
        </div>;
      })}
    </div>
