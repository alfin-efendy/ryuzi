...
async function parentListing(runnerId: string, sessionPk: string, parentDir: string): Promise<Entry> {
  const key = `${sessKey(runnerId, sessionPk)}:${parentDir}`;
  const hit = cache.get(key);
  if (hit && Date.now() - hit.at < TTL_MS) return hit;
  const pending = inFlight.get(key);
  if (pending) return pending;
  const fetch = (async () => {
    const res = await commands.listDir(runnerId, sessionPk, parentDir).catch(() => null);
...
export async function workspaceFileExists(runnerId: string, sessionPk: string, rel: string): Promise<boolean> {
  const slash = rel.lastIndexOf("/");
  const parentDir = slash >= 0 ? rel.slice(0, slash) : "";
  const name = slash >= 0 ? rel.slice(slash + 1) : rel;
  if (!name) return false;
  const listing = await parentListing(runnerId, sessionPk, parentDir);
