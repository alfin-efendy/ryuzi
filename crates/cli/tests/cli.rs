...
#[test]
fn doctor_prints_five_report_lines() {
    ...
    assert_eq!(
        lines.len(),
        5,
        "doctor must print exactly 5 lines, got: {stdout}"
    );
    ...
    assert!(lines[3].starts_with("acp:    "), "line 4: {}", lines[3]);
    assert!(lines[4].starts_with("doctor: "), "line 5: {}", lines[4]);
