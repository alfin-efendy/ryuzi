...
fn migrations() -> Migrations<'static> {
    Migrations::new(vec![
        ...
        // Plugin install ledger. `plugin_installs` is the authoritative record
        // of every installed skill pack / single skill (source, resolved
        // commit, content fingerprint, pin, trust acknowledgment); the on-disk
        // .ryuzi-skill.json stamp remains the loader's trust gate but is no
        // longer the record of record. `plugin_attach_status` holds the last
        // session-attach outcome per plugin for the doctor surface. IF NOT
        // EXISTS: the rewind-and-replay migration tests re-run this on an
...
mod tests {
    ...
    async fn migration_13_rewrites_claude_code_defaults_to_native() {
        ...
        // copy-drop-rename; 20 plugin_installs + plugin_attach_status —
        // CREATE TABLE IF NOT EXISTS; all no-ops on replay) re-run on the next
        // open. `Migrations` always fast-forwards to the latest defined
