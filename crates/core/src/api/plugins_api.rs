...
//! settings write (`set_plugin_setting`), plugin OAuth sign-in, and a
//! provider's effective model list (`plugin_models`). Moved (per the Move
//! Recipe) from `apps/cockpit/src-tauri/src/plugins_cmd.rs`; that file keeps
...
//!
//! Behavior change from the Tauri original: `begin_plugin_oauth` no longer
//! takes an `AppHandle` or opens the system browser directly — it broadcasts
//! [`CoreEvent::PluginOauthAuthorizeUrl`] via `state.cp.emit(..)` and Cockpit
//! opens the browser on receipt (Task 15).

use super::{ok, params, ApiError};
use crate::api::types::*;
use crate::control::ControlPlane;
use crate::domain::CoreEvent;
use crate::plugins::oauth::{generate_pkce_verifier, pkce_challenge_s256, PluginOauthToken};
