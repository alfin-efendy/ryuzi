...
# do. `plugins/openai` is a STANDALONE workspace crate (not a tests/fixtures
# fixture), so this is a sibling of build-components.sh rather than another
# `build_fixture` line. It touches only `plugins/openai/wit/deps` (gitignored),
# so it never races build-components.sh's `wit/deps` rewrites of the fixtures.

ROOT=$(CDPATH= cd -- "$(dirname -- "$0")/../../../.." && pwd)
PLUGIN="$ROOT/plugins/openai"
...
artifact="$PLUGIN/target/$TARGET/release/ryuzi_plugin_openai.wasm"
test -f "$artifact"
printf '%s\n' "$artifact"
