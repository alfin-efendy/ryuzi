...
# do. `plugins/discord` is a STANDALONE workspace crate (not a tests/fixtures
# fixture), so this is a sibling of build-components.sh (and
# build-github-component.sh) rather than another `build_fixture` line. It
# touches only `plugins/discord/wit/deps` (gitignored), so it never races
# build-components.sh's `wit/deps` rewrites of the fixtures.

ROOT=$(CDPATH= cd -- "$(dirname -- "$0")/../../../.." && pwd)
PLUGIN="$ROOT/plugins/discord"
...
artifact="$PLUGIN/target/$TARGET/release/ryuzi_plugin_discord.wasm"
test -f "$artifact"
printf '%s\n' "$artifact"
