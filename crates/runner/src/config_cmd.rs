...
async fn config_inner(args: &[String], deps: &mut Deps) -> u8 {
    // `ryuzi config` opens the settings store directly — it never builds a
    // full `Registries` (that would be unnecessary work just to read/write
    // settings). Without this, the process-wide `plugin.*` settings registry
    // stays empty here:
