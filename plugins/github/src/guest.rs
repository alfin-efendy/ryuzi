...
//! module) and sent through `oauth.authorized-request("github", ..)`, which
//! injects the bearer host-side; this component never sees a token. Responses
//! are handed straight back to [`logic::parse_response`].
...
/// `github` profile. The host injects the bearer and strips any component-set
/// `authorization` (never present here); the component never sees the token.
...
fn oauth_error_to_connector(tool: &str, error: oauth::OauthError) -> ConnectorError {
    match error {
        oauth::OauthError::Denied => ConnectorError::Failed(format!(
            "GitHub is not connected. Connect it via Cockpit's Plugins screen before calling '{tool}'."
        )),
        oauth::OauthError::Expired => ConnectorError::Failed(
            "GitHub authorization expired. Reconnect it via Cockpit's Plugins screen.".to_string(),
        ),
        oauth::OauthError::InvalidRequest(message) => ConnectorError::InvalidCall(message),
        oauth::OauthError::Failed(message) => {
            ConnectorError::Failed(format!("GitHub request failed: {message}"))
        }
    }
}
