//! The `anthropic` provider's wire logic: Anthropic **Messages**, not the
//! OpenAI-chat shape every other ported component speaks.
//!
//! Everything here is host-free and pure, so `cargo test` in this directory
//! covers the whole wire mapping natively on any target; the `wasm32`-only
//! [`crate::guest`] module is effect orchestration and WIT type mapping only.
//!
...
//! function rather than shared behaviour. `anthropic` is currently the only
//! component speaking this shape, so the logic lives here; if the OAuth
//! Anthropic variant lands it can lift this module into a sibling
//! `plugins/anthropic-format/` crate unchanged, exactly the way
//! `ryuzi-openai-format` was extracted.
//!
//! # Nothing here touches a credential
//! The host resolves the user's stored key and injects it per the descriptor's
//! `AuthScheme::XApiKey`. No function here sees, stores, or renders one — and
...
/// OPTIONAL base-URL override — the same product-level affordance every other
/// provider component exposes (pointing at a compatible gateway, and letting
/// the provider conformance harness aim the component at a loopback mock). A
/// blank/whitespace value is treated as "unset". The manifest network allowlist
/// still governs whatever the override resolves to, so an override can never
/// widen where the user's key may travel.
pub const BASE_URL_STORAGE_KEY: &str = "base-url";

/// The Anthropic API version this component pins, sent as the
/// `anthropic-version` request header on every call.
///
/// Anthropic REQUIRES this header and treats it as the contract version for
/// request and response shapes, so it must be a value chosen deliberately
/// rather than tracked implicitly. `2023-06-01` is the current stable Messages
/// version and is the SAME value the native router path already pins
/// (`llm_router::client::upstream_request` and
/// `llm_router::models::build_model_list_request`), so the component and the
/// native path cannot interpret the same account differently. It is a protocol
/// version, not a credential: the guest sets it, the host forwards it.
...
/// request — omitting it is a hard 400, so "leave it out when the caller did
/// not ask" is not an option here and a default must be picked. This matches
/// the default the engine's own OpenAI->Anthropic request translation already
...
/// cost of it being low is a truncated long answer; the cost of it being high
/// is nothing until a model actually generates that much. 4096 is the
/// conservative middle the rest of this codebase already settled on.
...
/// `display_name`, `created_at`, `type`), so a window is either a static hint
/// or a guess. This is the conservative hint, and it deliberately mirrors the
/// value the router itself already falls back to
/// (`llm_router::model_meta::FALLBACK.context_window`) rather than introducing
/// a second, differently-wrong default.
...
/// returns it for any unknown resource, but on this component's only two
...
/// (`crates/core/src/llm_router/registry.rs`) already states, so the config is
/// a transcription of the descriptor rather than an independent guess. The
/// struct exists (rather than bare constants) so a second Anthropic-shaped
/// provider — the OAuth variant, a gateway — is a new config value, not a fork
/// of this module.
...
impl AnthropicFormat {
    ...
    /// the request is exactly one `user` turn with STRING content — no system
    /// prompt, no tools, no content blocks. `stream` is false because the host
    /// capability is a buffered request/response: the component asks for the
    /// whole message and returns it as one terminal chunk.
    ///
    ...
    /// representation for those values, so there is nothing to send. The
    /// request still goes out and the upstream applies its own default —
    /// failing an entire completion over an unrepresentable optional tuning
    /// knob would be the worse trade.
    ...
    /// completion chunks: the FIRST `text` block of `content[]` becomes a
    /// single terminal chunk carrying the response's token usage when present.
    ///
    /// Anthropic returns `content` as an array of typed blocks. Only `text`
    /// blocks carry prose; a `thinking` or `tool_use` block that precedes one
    /// is skipped rather than rendered, so a reasoning model's private thinking
    /// is never surfaced as the completion. The flat ABI has no representation
    /// for anything past the first text block, which is a known consequence of
    /// that ABI (a plain, tool-free request returns exactly one text block).
...
/// is dropped.
///
/// This filter is this module's one non-obvious security-relevant behaviour: it
...
/// `x-api-key` per the descriptor, and this component has no `ryuzi:http`
/// import with which to set one itself.
///
...
/// windows are larger, but fabricating them here would be a guess presented as
/// a fact, and an over-stated window silently overflows prompts.
...
mod tests {
    ...
    /// is anti-tautology: assertions run against both configs, so a function
    /// that ignored `self` and hardcoded Anthropic's values would fail here even
    /// though it passed against [`CONFIG`].
    ...
    fn base_url_defaults_to_the_configured_api_and_honours_a_non_empty_override() {
        ...
        assert_eq!(
            CONFIG.resolve_base_url(Some("http://127.0.0.1:8080")),
            "http://127.0.0.1:8080"
        );
        assert_eq!(
            CONFIG.resolve_base_url(Some("https://gateway.test/v1/")),
    ...
    fn messages_body_maps_the_flat_prompt_to_a_single_user_turn() {
        let body: Value = serde_json::from_slice(&CONFIG.build_messages_body(
            "claude-sonnet-4-5",
            "ping",
            None,
            None,
        ...
        assert!(
            ...
            "the flat ABI has no system turn to send",
        );
    }
    ...
    fn messages_body_always_carries_max_tokens_defaulting_when_the_abi_omits_it() {
        ...
        let defaulted: Value =
            serde_json::from_slice(&CONFIG.build_messages_body("m", "hi", None, None)).unwrap();
        assert_eq!(defaulted["max_tokens"], DEFAULT_MAX_TOKENS);
        assert_eq!(DEFAULT_MAX_TOKENS, 4_096);

        let requested: Value =
            serde_json::from_slice(&CONFIG.build_messages_body("m", "hi", Some(64), Some(0.2)))
                .unwrap();
        assert_eq!(
        ...
        // into the builder.
        let other: Value =
            serde_json::from_slice(&OTHER.build_messages_body("m", "hi", None, None)).unwrap();
    ...
    fn messages_body_drops_a_non_finite_temperature_rather_than_failing() {
        for bad in [f32::NAN, f32::INFINITY, f32::NEG_INFINITY] {
            let body: Value =
                serde_json::from_slice(&CONFIG.build_messages_body("m", "hi", None, Some(bad)))
                    .unwrap();
            assert!(
    ...
    fn parse_models_maps_data_entries_preferring_anthropics_display_name() {
        ...
        let models = CONFIG.parse_models(body).unwrap();
    ...
    fn parse_models_rejects_a_body_without_a_data_array() {
        assert!(matches!(
            CONFIG.parse_models(b"not json"),
            Err(ProviderFail::Failed(_))
        ));
        assert!(matches!(
            CONFIG.parse_models(br#"{"has_more":false}"#),
            Err(ProviderFail::Failed(_))
        ));
    ...
    fn parse_message_response_yields_one_terminal_chunk_with_anthropic_usage() {
        ...
        let chunks = CONFIG.parse_message_response(body).unwrap();
    ...
    fn parse_message_response_ignores_an_openai_shaped_body() {
        // Guards against the two formats being crossed: an OpenAI chat
        // completion has `choices`, no `content[]`, and must NOT parse here.
        let body = br#"{"choices":[{"message":{"role":"assistant","content":"hi"}}],
                        "usage":{"prompt_tokens":1,"completion_tokens":2}}"#;
        assert!(matches!(
            CONFIG.parse_message_response(body),
            Err(ProviderFail::Failed(_))
        ));
    }
    ...
    fn parse_message_response_rejects_a_body_with_no_text_block() {
        assert!(matches!(
            CONFIG.parse_message_response(br#"{"content":[]}"#),
            Err(ProviderFail::Failed(_))
        ));
        assert!(matches!(
            CONFIG.parse_message_response(br#"{"content":[{"type":"tool_use","id":"t1"}]}"#),
            Err(ProviderFail::Failed(_))
        ));
        assert!(matches!(
            CONFIG.parse_message_response(b"not json"),
            Err(ProviderFail::Failed(_))
        ));
    }
    ...
    fn classify_error_maps_429_to_rate_limited_and_5xx_to_unavailable() {
        assert_eq!(CONFIG.classify_error(429, b""), ProviderFail::RateLimited);
        assert_eq!(
            CONFIG.classify_error(
                429,
    ...
    fn classify_error_maps_anthropics_not_found_type_to_model_not_found() {
        ...
        assert_eq!(
            CONFIG.classify_error(404, body),
        ...
        assert!(matches!(
            CONFIG.classify_error(404, br#"{"type":"error","error":{"type":"unknown_route"}}"#),
    ...
    fn a_classified_error_never_echoes_the_upstream_message_or_a_credential() {
        ...
        let rendered = format!("{:?}", CONFIG.classify_error(401, body));
        assert!(
