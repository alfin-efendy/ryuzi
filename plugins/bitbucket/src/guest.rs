...
//! module) and sent through `oauth.authorized-request("bitbucket-cloud", ..)`,
...
/// `bitbucket-cloud` profile. The host injects the bearer and strips any
/// component-set `authorization` (never present here); the component never
/// sees the token. DISTINCT from the `atlassian-cloud` profile the Atlassian
/// connector uses — this is the whole point of the two separate bundles.
...
fn oauth_error_to_connector(tool: &str, error: oauth::OauthError) -> ConnectorError {
    match error {
        oauth::OauthError::Denied => ConnectorError::Failed(format!(
            "Bitbucket is not connected. Connect it via Cockpit's Plugins screen before calling '{tool}'."
        )),
        oauth::OauthError::Expired => ConnectorError::Failed(
            "Bitbucket authorization expired. Reconnect it via Cockpit's Plugins screen."
                .to_string(),
        ),
        oauth::OauthError::InvalidRequest(message) => ConnectorError::InvalidCall(message),
        oauth::OauthError::Failed(message) => {
            ConnectorError::Failed(format!("Bitbucket request failed: {message}"))
        }
    }
}
